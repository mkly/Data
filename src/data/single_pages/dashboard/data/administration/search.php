	...
	<?php } else { ?>
		<?php $alt = ''; foreach ($dataTypes as $dataType) { ?>
			...
			<tr class="ccm-list-record<?= $alt ?>">
				...
				><?php if ($dataType->permissions->canEditDataTypePermissions()) { ?><?= t('Permissions') ?></a><?php } ?><a href="<?= $this->url('/dashboard/data/administration', $dataType->dtID) ?>"><?= $dataType->dtName ?></a><?php if ($dataType->permissions->canEditDataType()) { ?><?= $interface->button(t('Metadata'), $this->url('/dashboard/data/administration/edit', $dataType->dtID)) ?><?= $interface->button(t('Attributes'), $this->url('/dashboard/data/administration/attributes', $dataType->dtID)) ?><?php } ?></td>
			</tr>
